...
{
  int temp = x ^ (x << 1); //最高的不同位为1，合并正负数情况
  ...
  // 该部分参考 https://blog.csdn.net/perry0528/article/details/89425661
}
// float
/*
...
{
  unsigned a = (uf >> 23) & 0xff; //指数位
  unsigned flag = uf >> 31 << 31; //符号位
  int temp = uf;
  if (a == 0xff) // Nan：直接返回原数
    return uf;
  else if (a == 0) //弱规范数：小数部分左移一位
  {
    temp = uf << 1;
    temp = temp | flag;
  }
